...
IMPORTANT: You can find out the Inventory route url using `*odo url list --component service --app inventory`. Replace 
...
IMPORTANT: You can use `*curl -v*` to see all the headers sent and received. You would received 
IMPORTANT: a `*HTTP/1.1 204 No Content*` response for the above request.
...
IMPORTANT: via environment variables as we did previously. Read more in the https://access.redhat.com/documentation/en-us/red_hat_jboss_middleware_for_openshift/3/html/red_hat_java_s2i_for_openshift/reference#configuration_environment_variables[Java S2I Image docs^].
